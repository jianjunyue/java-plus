...
import com.java.plus.predict.load.ZipUtils;
 

public class TensorflowModel implements Serializable {
    ...
    public double[] output(double[][] mat) {
        try (Tensor<TFloat64> input = TFloat64.tensorOf(StdArrays.ndCopyOf(mat))) {
            Tensor<TFloat64> output = this.session.runner().feed(inputName, input).fetch(outputName).run().get(0).expect(TFloat64.DTYPE);
