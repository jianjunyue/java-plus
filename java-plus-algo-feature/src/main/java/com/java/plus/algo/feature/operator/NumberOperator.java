...
public class NumberOperator implements Serializable {
     ...
     * @return 当前所在的桶位置
     */
    public double cut(double num,int barrelSize,double min,double max){
     ...
     * @return
     */
    public double[] cutWithFill(double num, int barrelSize, double min, double max) {
     ...
     * @return
     */
    public double scaleZeroOne(double num,double min,double max){
        double div = max - min;
        if(num < min){
            return 0;
        }else if(num > max){
     ...
     * @return
     */
    public double div(double x,double y, double defaultValue){
        if(y == 0.0){
            return defaultValue;
        }else{
            return x/y;
     ...
     * [10000,inf) = 4
     */
    public double cutWithRange(double number, List<Double> binBound){
     ...
     */
    public double[] cutWithRangeAndOneHot(double number, List<Double> binBound) {
     ...
     * @return
     */
    public double[] cutWithRangeAndFill(double number, List<Double> binBound) {
        double v = cutWithRange(number, binBound);
        double[] result = new double[binBound.size() + 1];
     ...
     * @return
     */
    public double[] polynomial(int degree, double... rawData) {
        if (degree <= 1) {
            return rawData;
        }
     ...
     * @return
     */
    public double outlierFill(double feature, double min, double max, double defaultValue) {
     ...
     * @return
     */
    public double[] interWithFunc(String funcSign, double... features) {
     ...
     * 向量点乘
     */
    public double dot(List<Double> vector1, List<Double> vector2, double fillNa) {
        if (CollectionUtils.isEmpty(vector1) || CollectionUtils.isEmpty(vector2)) {
            return 0;
        }
        if (vector1.size() != vector2.size()) {
            throw new FeatureException("dot 算子要求长度匹配");
        }
        double result = 0;
