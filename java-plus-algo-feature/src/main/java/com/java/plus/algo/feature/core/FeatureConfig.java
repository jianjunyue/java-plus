...
public class FeatureConfig  implements Serializable {
     ...
     * @param allType 特征库
     */
    public FeatureConfig (int modelVersion, List<Feature> features, Map<String, RawFeatureType> allType) {
    ...
    public double[] transformToArray(Map<String, Object> rawDataMap) {
        ...
        for (FeatureExecutor featureExecutor : featureExecutors) {
            try {
                ...
                Map<String, Object> paramMap = Maps.newHashMapWithExpectedSize(rawList.size());
                for (String s : rawList) {
                    if (rawDataMap.containsKey(s)) {
                        Object rawData = rawDataMap.get(s);
                        if (rawData != null) {
                            switch (rawFeatureTypeMap.get(s)) {
                                case Number:
                                    if (StringUtils.isNotBlank(rawData.toString())) {
                                        paramMap.put(s, Double.valueOf(rawData.toString()));
                                    }
                                    break;
                                case Text:
                                    paramMap.put(s, String.valueOf(rawData));
                                    break;
                                default:
                                    log.error("data type not support, can check the jar version, or check the data source");
                                    throw new UnsupportedOperationException("不支持的原始数据类型");
                            }
                        }
                    }
                }
                double[] partVector = featureExecutor.toVector(paramMap);
